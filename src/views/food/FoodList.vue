  ...
  export default {
    ...
    methods: {
      async init () {
        ...
        await FoodApi.getDishPage(params).then(res => {
      ...
      addFoodtype (st) {
        if (st === 'add'){
          ...
          this.$store.state.foodUpdateId = st
          this.$router.push('/home/foodupdate')
        }
      },
      ...
      deleteHandle (type, id) {
        ...
        this.$confirm('确认删除该菜品, 是否继续?', '确定删除', {
          ...
          FoodApi.deleteDish(type === '批量' ? this.checkList.join(',') : id).then(res => {
            if (res.data.code === 1) {
      ...
      statusHandle (row) {
        ...
        this.$confirm('确认更改该菜品状态?', '提示', {
          ...
          // 起售停售---批量起售停售接口
          FoodApi.dishStatusByStatus(this.dishState).then(res => {
            if (res.data.code === 1) {
